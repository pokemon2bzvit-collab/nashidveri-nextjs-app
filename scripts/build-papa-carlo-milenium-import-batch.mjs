...
const scan = await scanMilenium({ offset, limit });
const models = scan.models.map((model, index) => ({ ...model, slug: `papa-carlo-${model.code.toLowerCase().replace(/\s+/g, "-")}-official`, sortOrder: 700 + offset + index }));
...
const products = models.map((model) => `(${[
  model.slug, "interior", "Papa Carlo", "Milenium", `Papa Carlo ${model.code}`, "Міжкімнатні", "Колекція Milenium", "Варіанти декорів і скла", "Ціна за запитом", description(model), JSON.stringify(["Фабрика Papa Carlo", "Колекція Milenium"]), model.images[0], model.sortOrder, true,
].map(sql).join(", ")})`);
...
console.log(`-- Papa Carlo Milenium: пакет ${offset + 1}–${offset + models.length} з ${scan.total}.`);
console.log("begin;");
