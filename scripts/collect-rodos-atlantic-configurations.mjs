...
const paintModels = [...(sitemapReport.matched || []), ...(sitemapReport.unmatched || [])]
  .filter((item) => /^Межкомнатная дверь Гранд Paint(?:\s|$)/u.test(item.title))
  .sort((left, right) => left.title.localeCompare(right.title, "uk", { numeric: true }))
  .map((item, index) => ({ code: `P${index + 1}`, slug: item.slug, url: item.url }));
const lineaModels = [...(sitemapReport.matched || []), ...(sitemapReport.unmatched || [])]
  .filter((item) => /^Межкомнатная дверь Linea\s+\d+$/u.test(item.title))
  .sort((left, right) => left.title.localeCompare(right.title, "uk", { numeric: true }))
  .map((item, index) => ({ code: `LN${index + 1}`, slug: item.slug, url: item.url }));
...
const translateColor = (value) => clean(value)
  .replace(/^Акация темная BLK$/iu, "Акація темна BLK")
  .replace(/^Акация темная$/iu, "Акація темна")
  .replace(/^Венге шоколадный BLK$/iu, "Венге шоколадний BLK")
  .replace(/^Венге шоколадный$/iu, "Венге шоколадний")
  .replace(/^Графит BLK$/iu, "Графіт BLK")
  .replace(/^Графит$/iu, "Графіт")
  .replace(/^Дуб шале графит BLK$/iu, "Дуб шале графіт BLK")
  .replace(/^Дуб шале графит$/iu, "Дуб шале графіт")
  .replace(/^Каштан белый BLK$/iu, "Каштан білий BLK")
  .replace(/^Каштан серый BLK$/iu, "Каштан сірий BLK")
  .replace(/^Кедр софт белый BLK$/iu, "Кедр софт білий BLK")
  .replace(/^Кедр софт белый$/iu, "Кедр софт білий")
  .replace(/^Кедр софт графит BLK$/iu, "Кедр софт графіт BLK")
  .replace(/^Кедр софт графит$/iu, "Кедр софт графіт")
  .replace(/^Мрамор сірий$/iu, "Мармур сірий")
  .replace(/^Светло серый BLK$/iu, "Світло-сірий BLK")
  .replace(/^[CС]ерый дуб BLK$/iu, "Сірий дуб BLK")
  .replace(/^[CС]ерый дуб$/iu, "Сірий дуб")
  .replace(/^Серый мрамор BLK$/iu, "Сірий мармур BLK")
  .replace(/^Сосна браш минт BLK$/iu, "Сосна браш мінт BLK")
  .replace(/^Цвет RAL\s+/iu, "Колір RAL ")
  .replace(/^Грунт$/iu, "Ґрунт")
