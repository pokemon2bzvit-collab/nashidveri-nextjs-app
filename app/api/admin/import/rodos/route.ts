...
function collectionFrom(value: string) { const x = value.toLocaleLowerCase("uk-UA"); if (/квартир|kvartir/.test(x)) return "Квартира"; if (/улиц|ulic/.test(x)) return "Вулиця"; if (/grand[- ]?delux|делюкс/.test(x)) return "DELUX"; if (/grand[- ]?lux|гранд люкс/.test(x)) return "LUX"; if (/grand[- ]?paint|гранд пейнт/.test(x)) return "Paint"; if (/linea/.test(x)) return "Linea"; if (/modern/.test(x)) return "Modern"; if (/atlantic/.test(x)) return "Atlantic ПВХ"; if (/cortes/.test(x)) return "Cortes фарба"; if (/siena/.test(x)) return "Siena фарба"; if (/royal/.test(x)) return "Royal шпон"; if (/loft/.test(x)) return "Loft фарба"; if (/style/.test(x)) return "Style ПВХ"; return "Новинки Rodos"; }
