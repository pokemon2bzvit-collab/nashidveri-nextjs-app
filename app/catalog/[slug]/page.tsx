...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    description: `${product.name}: ${product.description} Характеристики, доступні декори та консультація в салоні «Наші двері» в Ужгороді.`,
    alternates: { canonical: `/catalog/${product.slug}` },
    openGraph: { type: "website", title: `${product.name} — ${siteName}`, description: product.description, images: [{ url: absoluteUrl(product.image), alt: product.name }] },
  };
}
