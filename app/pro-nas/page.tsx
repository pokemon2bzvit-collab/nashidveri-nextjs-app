...
export default function AboutPage() {
  return <SiteShell><main>
    ...
    <section className="border-y border-stone-200 bg-sand py-16 sm:py-20"><div className="container-page"><p className="eyebrow">Як ми працюємо</p><h2 className="mt-3 max-w-2xl font-display text-4xl font-semibold tracking-[-.04em] text-ink sm:text-5xl">Спокійний шлях від ідеї до встановлених дверей.</h2><div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{steps.map((step) => <article key={step.number} className="rounded-2xl border border-stone-200 bg-white p-6 shadow-sm"><p className="text-sm font-bold text-clay">{step.number}</p><h3 className="mt-5 text-xl font-bold text-ink">{step.title}</h3><p className="mt-3 text-sm leading-6 text-stone-600">{step.text}</p></article>)}</div></div></section>
