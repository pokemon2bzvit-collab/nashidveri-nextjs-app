...
export function Footer() {
  ...
  return <footer className="border-t border-stone-200 bg-sand pb-20 pt-8 text-stone-600 sm:pb-8 sm:pt-10"><div className="container-page grid gap-7 sm:grid-cols-2 lg:grid-cols-[1.25fr_1fr_1fr]"><div><BrandMark /><p className="mt-3 max-w-xs text-sm leading-5">Гарантія вашого затишку. Двері й вікна з професійним підбором та монтажем в Ужгороді.</p><div className="mt-4 flex gap-2"><a aria-label="Facebook" href="https://www.facebook.com/nashidveriuz" target="_blank" rel="noreferrer" className="rounded-full border border-stone-300 bg-white p-2 transition hover:border-clay hover:text-clay"><Facebook size={16} /></a><a aria-label="Instagram" href="https://www.instagram.com/nashi_dveri_uzh/" target="_blank" rel="noreferrer" className="rounded-full border border-stone-300 bg-white p-2 transition hover:border-clay hover:text-clay"><Instagram size={16} /></a></div></div><div><p className="text-xs font-bold uppercase tracking-[.14em] text-stone-400">Навігація</p><nav className="mt-3 grid grid-cols-2 gap-x-5 gap-y-2.5 text-sm font-medium lg:grid-cols-3">{links.map(([label, href]) => <Link key={href} href={href} className="w-fit transition hover:text-clay">{label}</Link>)}</nav></div><div><p className="text-xs font-bold uppercase tracking-[.14em] text-stone-400">Зв’язок</p><div className="mt-3 grid gap-2.5 text-sm font-medium"><a href="tel:+380950729341" className="flex w-fit items-center gap-2 transition hover:text-clay"><Phone size={16} className="text-clay" />+38 (095) 072-93-41</a><a href="mailto:nashidveri.uzh@gmail.com" className="flex w-fit items-center gap-2 transition hover:text-clay"><Mail size={16} className="text-clay" />Написати нам</a><Link href="/contacts" className="mt-1 inline-flex w-fit items-center rounded-full bg-ink px-4 py-2 text-xs font-bold text-white transition hover:bg-clay">Замовити замір</Link></div></div></div><div className="container-page mt-7 flex flex-col gap-2 border-t border-stone-200 pt-4 text-xs text-stone-500 sm:flex-row sm:items-center sm:justify-between"><span>© НАШІ ДВЕРІ. Всі права застережено</span><span>Ужгород · вул. Івана Чендея, 44</span></div></footer>;
}
