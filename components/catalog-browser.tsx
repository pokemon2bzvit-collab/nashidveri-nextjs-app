...
export function CatalogBrowser({ initialData, initialCategory = "all", initialQuery = "", initialBrand = "all", initialCollection = "all" }: { initialData: CatalogBrowseData; initialCategory?: string; initialQuery?: string; initialBrand?: string; initialCollection?: string }) {
  ...
  return <div className="mt-0 grid gap-8 lg:grid-cols-[255px_minmax(0,1fr)]">
    <button aria-label="Відкрити фільтри" className="fixed left-0 top-1/2 z-30 flex w-10 -translate-y-1/2 justify-center rounded-r-xl bg-ink py-3 text-[10px] font-bold text-white shadow-lg lg:hidden" onClick={() => setFiltersOpen(true)}><span className="[writing-mode:vertical-rl]">Фільтри</span></button>
    ...
    <aside className={`surface-card fixed inset-y-0 left-0 z-50 h-full w-[min(88vw,330px)] overflow-y-auto p-5 shadow-2xl transition-transform lg:sticky lg:top-28 lg:z-auto lg:h-[calc(100vh-8rem)] lg:w-auto lg:translate-x-0 lg:overflow-y-auto lg:pr-3 lg:shadow-none ${filtersOpen ? "translate-x-0" : "-translate-x-full"}`}>
