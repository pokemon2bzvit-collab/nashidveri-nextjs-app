...
export function MarketDveriPreview({ accessToken, product }: { accessToken: string; product: Product }) {
  ...
  async function findUrl(kind: SourceKind) {
    if (!search.trim()) return;
    setFinding(true); setCandidates([]); setSearched(false); setMessage("");
    try {
      const endpoint = kind === "rodos" ? "/api/admin/import/rodos/search?name=" + encodeURIComponent(search.trim()) : "/api/admin/import/market-dveri/search?name=" + encodeURIComponent(search.trim()) + "&brand=" + encodeURIComponent(product.brand);
  ...
  return <section className="rounded-2xl border border-sky-200 bg-sky-50/50 p-5">
    ...
    <h3 className="mt-1 font-display text-2xl">Характеристики з джерела</h3>
    <p className="mt-2 max-w-2xl text-sm leading-6 text-stone-600">Спершу знайдіть точну картку Rodos або Market Dveri. Параметри й опис лише переглядаються — нічого в товарі не зберігається.</p>
