...
export function ProductCatalog({ onOrder }: { onOrder: () => void }) { const [tab, setTab] = useState<keyof typeof products>("interior"); return <section id="catalog" className="section-pad container-page"><div className="flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><p className="eyebrow">Наш каталог</p><h2 className="heading mt-3">Відкрийте двері<br />до вашого дому</h2></div><p className="max-w-sm text-sm leading-6 text-stone-600">Підібрали моделі, у яких дизайн зустрічається з надійністю. Більше варіантів покажемо у салоні.</p></div><div className="mt-10 flex w-full gap-2 overflow-x-auto border-b pb-3">{tabs.map(item => <button key={item.id} onClick={() => setTab(item.id)} className={`whitespace-nowrap rounded-full px-5 py-2.5 text-sm font-semibold transition ${tab === item.id ? "bg-ink text-white" : "text-stone-500 hover:bg-stone-100"}`}>{item.label}</button>)}</div><div className="mt-7 grid gap-5 md:grid-cols-3">{products[tab].map((product, index) => <article className="group overflow-hidden rounded-2xl border bg-white shadow-soft" key={product.name}><Link href={`/catalog/${product.name.toLowerCase().replaceAll(" ", "-")}`}><div className="relative aspect-[1.12] overflow-hidden"><img className="h-full w-full object-cover transition duration-500 group-hover:scale-105" src={product.image} alt={product.name} /><span className="absolute left-4 top-4 rounded-full bg-white/90 px-3 py-1 text-xs font-bold">0{index + 1}</span></div><div className="p-5"><p className="text-lg font-bold">{product.name}</p><p className="mt-1 text-sm text-stone-500">{product.detail}</p><div className="mt-5 flex items-center justify-between"><span className="font-display text-xl">{product.price}</span><span className="rounded-full border p-2.5 transition group-hover:bg-ink group-hover:text-white"><ArrowUpRight size={18} /></span></div></div></Link></article>)}</div><div className="mt-8 flex justify-center gap-6"><Link href="/catalog" className="inline-flex items-center gap-1 text-sm font-bold text-clay underline-offset-4 hover:underline">Весь каталог <ChevronRight size={16} /></Link><button onClick={onOrder} className="text-sm font-bold text-stone-600 hover:text-clay">Допоможемо підібрати модель</button></div></section> }
