...
export function DarumiImporter() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      supabase.from("products").select("slug").eq("brand", "Darumi"),
  ...
  async function addDraft() {
    ...
    try {
      ...
      const requests: PromiseLike<{ error: { message: string } | null }>[] = [
        supabase.from("product_sources").insert({ product_slug: slug, source_name: "Darumi", source_url: preview.sourceUrl, source_product_name: preview.title, verification_status: "verified", verified_at: new Date().toISOString(), notes: "Імпортовано з офіційної картки darumi.in.ua. Чернетка потребує перевірки перед публікацією." }),
      ];
      ...
      setNotice("Чернетку Darumi створено: фото, опис, характеристики та поточний варіант кольору/скла додані. Покупці її не бачать, доки ви не увімкнете показ.");
