...
export function ProductMediaGallery({ product }: { product: Product }) {
  ...
  return <div>
    <div className="aspect-[4/5] overflow-hidden rounded-[2rem] bg-[#f7f5f1] p-5 sm:p-8">
      <ImageLightbox src={selected.image} alt={selected.label || product.name} className="h-full w-full" imageClassName="h-full w-full object-contain" />
    </div>
    {gallery.length > 1 && <div className="mt-3 flex gap-2 overflow-x-auto pb-1">
      {gallery.map((item, index) => <button type="button" key={`${item.image}-${index}`} aria-label={`Обрати фото: ${item.label || index + 1}`} onClick={() => setSelectedIndex(index)} className={`h-16 w-12 shrink-0 overflow-hidden rounded-lg border-2 bg-[#f7f5f1] transition ${selectedIndex === index ? "border-clay" : "border-transparent hover:border-stone-300"}`}><img src={item.image} alt="" className="h-full w-full object-contain" /></button>)}
    </div>}
