...
function AdminOverviewContent() {
  ...
  <section className="mt-5 rounded-2xl border bg-white p-5 shadow-sm"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[.14em] text-clay">Поповнення каталогу</p><h2 className="mt-1 font-display text-3xl">Наші виробники</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-stone-600">Оберіть фабрику → перевірте, чого бракує → оновіть модель або додайте нову. Дані не потраплять на сайт без вашого підтвердження.</p></div><Link href="/admin/catalog" className="button-light px-3 py-2 text-sm">Усі товари <ArrowRight size={16} /></Link></div><div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{manufacturerHub.map((manufacturer) => { const factoryProducts = products.filter((product) => product.brand === manufacturer.brand); const count = factoryProducts.length; const needsCheck = factoryProducts.filter((product) => !product.image_path?.trim() || !product.description?.trim() || !sourcedSlugs.has(product.slug) || !specifiedSlugs.has(product.slug)).length; const ready = count - needsCheck; const href = "/admin/catalog?brand=" + encodeURIComponent(manufacturer.brand) + (manufacturer.importer ? "&importer=" + manufacturer.importer : ""); return <Link key={manufacturer.brand} href={href} className="group rounded-xl border border-stone-200 bg-stone-50 p-4 transition hover:-translate-y-0.5 hover:border-clay hover:bg-sand"><span className="grid size-10 place-items-center rounded-xl bg-white text-clay shadow-sm"><PackageSearch size={18} /></span><div className="mt-4 flex items-start justify-between gap-3"><div><h3 className="font-display text-xl">{manufacturer.brand}</h3><p className="mt-1 text-sm leading-5 text-stone-600">{manufacturer.note}</p></div><b className="shrink-0 rounded-full bg-white px-2.5 py-1 text-xs text-stone-600">{count}</b></div><div className="mt-4 flex flex-wrap gap-1.5 text-xs font-bold"><span className="rounded-full bg-green-100 px-2 py-1 text-green-800">Готово: {ready}</span>{needsCheck > 0 ? <span className="rounded-full bg-amber-100 px-2 py-1 text-amber-800">Перевірити: {needsCheck}</span> : <span className="rounded-full bg-stone-200 px-2 py-1 text-stone-600">Все заповнено</span>}</div><span className="mt-4 inline-flex items-center gap-2 text-sm font-bold text-clay">{manufacturer.importer ? "Сканувати та імпортувати" : "Керувати моделями"}<ArrowRight className="transition group-hover:translate-x-1" size={16} /></span></Link>; })}</div></section>
