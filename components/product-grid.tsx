...
import { catalogCardImage, catalogThumbnailUrl, categories, type CatalogCardProduct, type Product, type ProductOption, type ProductSpec } from "@/lib/catalog";
...
function DecorPreview({ option }: { option: Pick<ProductOption, "image" | "swatch"> }) {
  if (option.image) return <img src={catalogThumbnailUrl(option.image, 96)} alt="" className="h-full w-full object-cover" />;
  ...
  return <div className={singleColumn ? "grid grid-cols-1" : "grid grid-cols-2 gap-3 sm:gap-5 lg:grid-cols-3"}>
    {products.map((product) => { const hasPrice = product.price !== "Ціна за запитом"; const isCatalogCard = "decorOptions" in product; const image = isCatalogCard ? product.image : catalogCardImage(product); const decorOptions = isCatalogCard ? product.decorOptions : visualDecorOptions(product); const highlights = isCatalogCard ? product.highlights : productHighlights(product); const specs = isCatalogCard ? product.keySpecs : keySpecs(product.specs); return <article key={product.slug} className="group flex min-w-0 flex-col overflow-hidden rounded-2xl border border-stone-200/80 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:border-stone-300 hover:shadow-xl">
      <Link href={`/catalog/${product.slug}`} className="relative aspect-[4/5] overflow-hidden bg-[#f7f5f1] p-3 sm:p-5" aria-label={`Детальніше: ${product.name}`}>
          <img loading="lazy" decoding="async" src={catalogThumbnailUrl(image)} alt={productImageAlt(product)} className="h-full w-full object-contain transition duration-500 group-hover:scale-[1.035]" />
      ...
      <Link href={`/catalog/${product.slug}`} className="flex flex-1 flex-col" aria-label={`Детальніше: ${product.name}`}>
        <div className="flex flex-1 flex-col p-3 sm:p-5">
          ...
          <p className="mt-3 hidden line-clamp-2 min-h-10 text-xs leading-5 text-stone-600 sm:block">{product.description}</p>
          ...
          <div className="mt-3 flex items-end justify-between border-t border-stone-100 pt-3 sm:mt-5 sm:pt-4">
            ...
            <span className="hidden items-center gap-1 rounded-full bg-sand px-3 py-2 text-xs font-bold text-ink transition group-hover:bg-ink group-hover:text-white sm:inline-flex">Детальніше <Sparkles size={12} /></span>
          </div>
        </div>
      </Link>
      {showCart && <div className="border-t border-stone-100 px-3 py-2.5 sm:px-5 sm:py-3"><ProductCartButton compact item={{ slug: product.slug, name: product.name, brand: product.brand, collection: product.collection, image }} /></div>}
    </article>})}
  </div>;
}
