...
export function HomeCategories() {
  return <section className="container-page py-14 sm:py-20">
    ...
    <div className="mt-7 grid gap-3 sm:grid-cols-2">
      <Link href="/mizhkimnatni-dveri" className="group flex items-center justify-between rounded-2xl border border-stone-200 bg-white px-5 py-4 transition hover:border-clay hover:shadow-soft">
        <span><span className="block text-sm font-bold text-ink">Міжкімнатні двері</span><span className="mt-1 block text-xs text-stone-500">Для інтер’єру та затишку</span></span>
        <ArrowUpRight size={18} className="text-clay transition group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
      </Link>
      <Link href="/vhidni-dveri" className="group flex items-center justify-between rounded-2xl border border-stone-200 bg-white px-5 py-4 transition hover:border-clay hover:shadow-soft">
        <span><span className="block text-sm font-bold text-ink">Вхідні двері</span><span className="mt-1 block text-xs text-stone-500">Для квартири та будинку</span></span>
        <ArrowUpRight size={18} className="text-clay transition group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
      </Link>
    </div>
    <div className="mt-9 space-y-8">
      {groups.map((group) => <div key={group.title}><div className="flex items-center justify-between"><h3 className="font-display text-2xl tracking-[-.035em] text-ink sm:text-3xl">{group.title}</h3><Link href={group.href} className="text-sm font-bold text-clay hover:text-ink">Усі фабрики</Link></div><div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">{group.brands.map((brand) => <Link key={brand.slug} href={`${group.prefix}/${brand.slug}`} className="group rounded-2xl border border-stone-200 bg-white p-4 transition hover:-translate-y-0.5 hover:border-clay hover:shadow-soft sm:p-5"><BrandLogo brand={brand.name} className="h-12" /> <p className="mt-3 text-sm font-bold text-ink">{brand.name}</p><p className="mt-2 line-clamp-2 min-h-10 text-xs leading-5 text-stone-500">{brand.collections.slice(0, 2).join(" · ")}</p><span className="mt-5 inline-flex items-center gap-1 text-xs font-bold text-clay">Колекції <ArrowUpRight size={14} className="transition group-hover:translate-x-0.5 group-hover:-translate-y-0.5" /></span></Link>)}</div></div>)}
    </div>
