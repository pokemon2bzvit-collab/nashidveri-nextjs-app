...
export function Header({ onMeasure }: { onMeasure: () => void }) {
  ...
  return <header className="sticky top-0 z-40 border-b border-stone-200 bg-white/95 text-ink backdrop-blur-xl">
    ...
    <div className="container-page flex h-16 items-center justify-between gap-4"><Link href="/" className="shrink-0"><BrandMark /></Link><nav className="hidden items-center gap-7 xl:flex">{links.map(([label, href]) => <Link key={label} href={href} className="text-xs font-medium text-stone-600 transition hover:text-ink">{label}</Link>)}</nav><div className="hidden items-center gap-4 lg:flex"><a href="tel:+380950729341" className="inline-flex items-center gap-1.5 text-xs text-stone-600"><Phone size={13} />+38 (095) 072-93-41</a><button onClick={onMeasure} className="rounded-full bg-ink px-3.5 py-1.5 text-xs font-bold text-white transition hover:bg-clay">Замовити замір</button></div><button aria-label="Відкрити меню" className="lg:hidden" onClick={() => setOpen(!open)}>{open ? <X size={20} /> : <Menu size={20} />}</button></div>
