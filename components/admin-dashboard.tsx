...
function JsonEditor({ title, help, value, onChange, onSave }: { title: string; help: string; value: string; onChange: (value: string) => void; onSave: () => void }) { return <div className="rounded-2xl border bg-white p-5 shadow-sm"><SectionTitle icon={<Check size={18} />} title={title} /><p className="mt-2 text-sm text-stone-500">{help}</p><textarea className={`${areaClass} mt-4 font-mono text-xs`} value={value} onChange={(event) => onChange(event.target.value)} /><button onClick={onSave} className="button-primary mt-4"><Save size={16} /> Зберегти</button></div>; }
