...
export function AdminDashboard() {
  ...
  return <main className="min-h-screen bg-stone-50 text-ink">
    <header className="sticky top-0 z-20 border-b bg-white/95 backdrop-blur"><div className="mx-auto flex max-w-[1600px] items-center justify-between gap-4 px-4 py-3 sm:px-6"><div><p className="text-[11px] font-bold uppercase tracking-[.16em] text-clay">Закрита адмінка</p><h1 className="font-display text-xl sm:text-2xl">Каталог «Наші двері»</h1></div><div className="flex items-center gap-2"><a href="/admin" className="button-light px-3 py-2 text-sm">Огляд</a><button onClick={() => supabase.auth.signOut()} className="button-light px-3 py-2 text-sm"><LogOut size={16} /> Вийти</button></div></div></header>
