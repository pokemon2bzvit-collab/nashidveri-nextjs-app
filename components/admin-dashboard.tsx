...
type Source = {
    ...
    source_name: string;
    source_url: string;
    verification_status: "verified" | "review" | "rejected";
};
const catalogSources = [
    { name: "Market Dveri", description: "Описи, характеристики та фото багатьох моделей", url: "https://market-dveri.ua/uk/" },
    { name: "Abwehr", description: "Офіційний каталог вхідних дверей", url: "https://abwehr.com.ua/" },
    { name: "Grand", description: "Офіційний каталог фабрики Grand", url: "https://www.granddoor.com.ua/#product" },
    { name: "Papa Carlo", description: "Офіційний каталог міжкімнатних дверей", url: "https://papa-carlo.com.ua/ua/" },
    { name: "Qdoors", description: "Офіційний каталог Qdoors", url: "https://qdoors-dveri.com.ua/dveri/" },
    { name: "Страж", description: "Офіційний каталог вхідних дверей", url: "https://straj.ua/" },
];
...
function safeJson<T>(value: string, title: string): T | null {
    try {
        ...
        alert(`${title}: виправте JSON перед збереженням.`);
        return null;
    }
}
export function AdminDashboard() {
    ...
    async function saveProduct() {
        ...
        if (!error) {
            const updated = { ...selected, ...draft } as ProductRow;
            setSelected(updated);
    ...
    async function replaceJsonRows(table: "product_specs" | "product_options" | "product_variants", text: string, title: string) {
        if (!selected)
            return;
        const rows = safeJson<Record<string, unknown>[]>(text, title);
        if (!rows)
            return;
        setSaving(true);
    ...
    async function addMedia(path: string) {
        ...
        setSaving(false);
        setNotice(error ? error.message : "Фото додано.");
        if (!error) {
            setMediaUrl("");
            setMediaLabel("");
    ...
    async function removeMedia(id: string) {
        ...
        setNotice(error ? error.message : "Запис медіа видалено.");
        if (!error && selected)
            await selectProduct(selected);
    }
    async function addSource() {
        ...
        setNotice(error ? error.message : "Джерело збережено.");
        if (!error) {
            setSourceUrl("");
            await selectProduct(selected);
        }
...
function JsonEditor({ title, help, value, onChange, onSave }: {
    title: string;
    help: string;
    value: string;
    ...
    return <>
        {kind === "variants" && <SourceLibrary />}
        <FriendlyEditor title={title} help={friendlyHelp || help} value={value} onChange={onChange} onSave={onSave} kind={kind}/>
    </>;
}

function SourceLibrary() {
    return <section className="rounded-2xl border bg-white p-5 shadow-sm">
        <SectionTitle icon={<Link2 size={18}/>} title="Бібліотека джерел" />
        <p className="mt-2 text-sm leading-6 text-stone-500">Відкрийте потрібний каталог, знайдіть точну модель і вставте її посилання в блок «Перевірені джерела» нижче. Так ми не змішуємо дані схожих моделей.</p>
        <div className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
            {catalogSources.map((source) => <a key={source.name} href={source.url} target="_blank" rel="noreferrer" className="rounded-xl border border-stone-200 bg-stone-50 p-3 transition hover:border-clay hover:bg-sand">
                <b className="block text-sm text-ink">{source.name}</b>
                <span className="mt-1 block text-xs leading-5 text-stone-500">{source.description}</span>
            </a>)}
        </div>
    </section>;
}

function FriendlyEditor({ title, help, value, onChange, onSave, kind }: {
    title: string;
    help: string;
    value: string;
    onChange: (value: string) => void;
    ...
    catch { /* JSON is available in advanced mode for repair. */ }
    ...
    const add = () => {
        if (kind === "specs")
            write([...rows, { label: "Нова характеристика", value: "", sort_order: rows.length * 10 + 10 }]);
        if (kind === "options")
            write([...rows, { option_group: "color", group_label: "Колір", label: "Новий декор", swatch: "#d8d1c5", image_path: null, sort_order: rows.length + 1, is_active: true }]);
        if (kind === "variants")
            write([...rows, { selections: { color: "Новий декор" }, image_path: "", sort_order: rows.length + 1, is_active: true }]);
    };
