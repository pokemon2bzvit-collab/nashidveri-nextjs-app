...
import { Check, ChevronLeft, ChevronRight, ExternalLink, FileImage, Link2, LoaderCircle, LogOut, PackageSearch, Paintbrush, Plus, Save, Search, ShieldCheck, SlidersHorizontal, Trash2, Upload, WandSparkles } from "lucide-react";
import { useRouter, useSearchParams } from "next/navigation";
import type { Session } from "@supabase/supabase-js";
import { QdoorsReview, type ImportFields } from "@/components/qdoors-review";
import { MarketDveriPreview } from "@/components/market-dveri-preview";
import { ProductGrid } from "@/components/product-grid";
import { catalogImageUrl, type Category, type Product as CatalogProduct, type ProductOption, type ProductSpec, type ProductVariant } from "@/lib/catalog";
import { AdminNavigation } from "@/components/admin-navigation";
import { adminEmail, getSupabaseBrowserClient } from "@/lib/supabase-browser";
import { createProductDescription } from "@/lib/product-description";
...
type QdoorsCatalogItem = { url: string; title: string };
...
function qdoorsTokens(value: string) {
  const ignored = new Set(["qdoors", "doors", "двері", "дверь", "ручка", "вулиця", "квартира", "фарба", "гладь", "біла", "білий", "антрацит", "ral"]);
  return Array.from(new Set(value.toLocaleLowerCase("uk-UA").replace(/[^a-zа-яіїєґ0-9]+/giu, " ").split(" ").filter((word) => word.length >= 3 && !ignored.has(word))));
}
type QdoorsMatch = { product: Product; confidence: "exact" | "possible" };
function qdoorsTokensMatch(left: string, right: string) {
  return left === right || (left.length >= 6 && right.length >= 6 && (left.startsWith(right) || right.startsWith(left)));
}
function getQdoorsMatch(title: string, products: Product[]): QdoorsMatch | null {
  const candidate = qdoorsTokens(title);
  const series = new Set(["стріт", "авангард", "преміум", "ультра"]);
...
function CatalogCardPreview({ selected, draft, specsText, optionsText, variantsText }: { selected: Product; draft: Partial<Product>; specsText: string; optionsText: string; variantsText: string }) {
  const specs = readRows(specsText).map((row, index) => ({
    label: String(row.label || "Характеристика"), value: String(row.value || ""), sortOrder: Number(row.sort_order ?? index),
  })) as ProductSpec[];
  const options = readRows(optionsText).map((row, index) => ({
    group: row.option_group === "finish" || row.option_group === "glass" || row.option_group === "edge" || row.option_group === "configuration" ? row.option_group : "color",
    groupLabel: String(row.group_label || "Декор"), label: String(row.label || "Варіант"), swatch: typeof row.swatch === "string" ? row.swatch : null,
    image: typeof row.image_path === "string" && row.image_path ? catalogImageUrl(row.image_path) : null, sortOrder: Number(row.sort_order ?? index),
  })) as ProductOption[];
  const variants = readRows(variantsText).filter((row) => typeof row.image_path === "string" && row.image_path).map((row, index) => ({
    selections: typeof row.selections === "object" && row.selections ? row.selections as Record<string, string> : {}, image: catalogImageUrl(String(row.image_path)), sortOrder: Number(row.sort_order ?? index),
  })) as ProductVariant[];
  const image = draft.image_path?.trim() || selected.image_path?.trim() || "";
  const product: CatalogProduct = {
    slug: selected.slug, category: selected.category as Category, brand: selected.brand, collection: selected.collection, name: selected.name,
...
export function AdminDashboard() {
  ...
  const filtered = products.filter((item) => {
    ...
    const matchesQuality = quality === "all"
      ...
      || (quality === "incomplete" && (!hasPhoto || !hasDescription || !sourcedSlugs.has(item.slug)))
  ...
  useEffect(() => {
    ...
    if (requested === "photo" || requested === "description" || requested === "source" || requested === "incomplete" || requested === "hidden") setQuality(requested);
    else setQuality("all");
  }, [searchParams]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (requestedProduct && selected?.slug !== requestedProduct.slug) choose(requestedProduct);
  }, [searchParams, products, selected?.slug]);
  ...
  function generateDescription() {
    if (!selected) return;
    setDraft({ ...draft, description: createProductDescription(selected, readRows(specsText), readRows(optionsText)) });
    setNotice("Чернетку опису створено з характеристик. Перегляньте текст і збережіть основні дані.");
  }
  ...
  return <main className="min-h-screen bg-stone-50 text-ink">
    <header className="sticky top-0 z-20 border-b bg-white/95 backdrop-blur"><div className="mx-auto flex max-w-[1600px] flex-wrap items-center justify-between gap-3 px-4 py-3 sm:px-6"><a href="/admin" className="shrink-0"><p className="text-[11px] font-bold uppercase tracking-[.16em] text-clay">Закрита адмінка</p><h1 className="font-display text-xl sm:text-2xl">Наші двері</h1></a><AdminNavigation /><button onClick={() => supabase.auth.signOut()} className="button-light px-3 py-2 text-sm"><LogOut size={16} /> Вийти</button></div></header>
    <div className="mx-auto max-w-[1600px] p-4 sm:p-6">
      ...
      <div className="grid gap-5 xl:grid-cols-[330px_minmax(0,1fr)]">
        ...
        {!selected ? <section className="grid min-h-80 place-items-center rounded-2xl border border-dashed bg-white p-8 text-center"><div><PackageSearch className="mx-auto text-clay" size={34} /><h2 className="mt-4 font-display text-3xl">Оберіть модель</h2><p className="mt-2 max-w-sm text-sm leading-6 text-stone-500">Скористайтеся пошуком або фабрикою, а потім натисніть на товар у списку.</p></div></section> : <section className="min-w-0">
          ...
          <nav className="mt-4 flex gap-2 overflow-x-auto pb-1">{([["basic", "Основне", <SlidersHorizontal key="basic" size={16} />], ["media", "Фото", <FileImage key="media" size={16} />], ["configuration", "Характеристики й декори", <Paintbrush key="configuration" size={16} />], ["sources", "Джерела", <Link2 key="sources" size={16} />]] as const).map(([id, label, icon]) => <button key={id} onClick={() => setTab(id)} className={"inline-flex shrink-0 items-center gap-2 rounded-full px-4 py-2.5 text-sm font-bold " + (tab === id ? "bg-ink text-white" : "border bg-white text-stone-600")}>{icon}{label}</button>)}</nav>
...
function SourceLibrary() {
  return <Card icon={<Link2 size={19} />} title="Бібліотека джерел" help="Відкрийте каталог, знайдіть точну модель і збережіть URL в джерелах. Загальна сторінка фабрики не підтверджує конкретний декор.">
    ...
    <a href="/admin/importers" className="mt-4 inline-flex text-sm font-bold text-clay underline">Перейти до імпорту та XML-файлів →</a>
  </Card>;
}
