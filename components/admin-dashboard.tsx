...
export function AdminDashboard() {
  ...
  if (!session) return <main className="grid min-h-screen place-items-center bg-stone-50 p-5"><form onSubmit={login} className="w-full max-w-md rounded-3xl border bg-white p-7 shadow-lg"><ShieldCheck className="text-clay" size={30} /><p className="mt-5 text-xs font-bold uppercase tracking-[.16em] text-clay">Адмінка каталогу</p><h1 className="mt-2 font-display text-4xl">Наші двері</h1><p className="mt-3 text-sm leading-6 text-stone-600">Введіть пошту адміністратора — ми надішлемо безпечне посилання для входу.</p><label className="mt-6 block text-sm font-bold">Електронна пошта<input className={inputClass} type="email" required value={email} onChange={(event) => setEmail(event.target.value)} placeholder="name@example.com" /></label><button className="button-primary mt-5 w-full" disabled={saving}>{saving ? "Надсилання…" : "Надіслати посилання"}</button>{notice && <p className="mt-4 rounded-xl bg-sand p-3 text-sm">{notice}</p>}</form></main>;
  ...
  return <main className="min-h-screen bg-stone-50 text-ink">
    <header className="sticky top-0 z-20 border-b bg-white/95 backdrop-blur"><div className="mx-auto flex max-w-[1600px] items-center justify-between gap-4 px-4 py-3 sm:px-6"><div><p className="text-[11px] font-bold uppercase tracking-[.16em] text-clay">Закрита адмінка</p><h1 className="font-display text-xl sm:text-2xl">Каталог «Наші двері»</h1></div><button onClick={() => supabase.auth.signOut()} className="button-light px-3 py-2 text-sm"><LogOut size={16} /> Вийти</button></div></header>
      ...
      <div className="grid gap-5 xl:grid-cols-[330px_minmax(0,1fr)]">
        ...
        {!selected ? <section className="grid min-h-80 place-items-center rounded-2xl border border-dashed bg-white p-8 text-center"><div><PackageSearch className="mx-auto text-clay" size={34} /><h2 className="mt-4 font-display text-3xl">Оберіть модель</h2><p className="mt-2 max-w-sm text-sm leading-6 text-stone-500">Скористайтеся пошуком або фабрикою, а потім натисніть на товар у списку.</p></div></section> : <section className="min-w-0">
          ...
          {loadingProduct ? <div className="mt-4 grid min-h-72 place-items-center rounded-2xl border bg-white text-stone-500"><span className="inline-flex items-center gap-2"><LoaderCircle className="animate-spin" size={18} /> Завантажуємо модель…</span></div> : <div className="mt-4">
            ...
            {tab === "configuration" && <Configuration specsText={specsText} optionsText={optionsText} variantsText={variantsText} setSpecsText={setSpecsText} setOptionsText={setOptionsText} setVariantsText={setVariantsText} onSave={saveRows} />}
...
function Configuration({ specsText, optionsText, variantsText, setSpecsText, setOptionsText, setVariantsText, onSave }: { specsText: string; optionsText: string; variantsText: string; setSpecsText: (value: string) => void; setOptionsText: (value: string) => void; setVariantsText: (value: string) => void; onSave: (table: "product_specs" | "product_options" | "product_variants", text: string, title: string) => void }) {
  ...
  return <div className="space-y-4"><div className="flex flex-wrap gap-2">{([["specs", "Характеристики"], ["options", "Декори й опції"], ["variants", "Фото варіантів"]] as const).map(([id, title]) => <button key={id} onClick={() => setKind(id)} className={"rounded-full px-4 py-2 text-sm font-bold " + (kind === id ? "bg-clay text-white" : "border bg-white text-stone-600")}>{title}</button>)}</div>{kind === "variants" && <SourceLibrary />}<RowsEditor kind={kind} title={config.title} help={config.help} value={config.value} onChange={config.change} onSave={() => onSave(config.table, config.value, config.title)} /></div>;
}
function RowsEditor({ kind, title, help, value, onChange, onSave }: { kind: EditorKind; title: string; help: string; value: string; onChange: (value: string) => void; onSave: () => void }) {
