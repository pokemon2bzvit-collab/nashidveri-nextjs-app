...
export function CartDrawer() {
  ...
  async function submit(values: Values) {
    const models = items.map((item, index) => {
      const configuration = item.configuration?.length ? `; ${item.configuration.join(", ")}` : "";
      return `${index + 1}. ${item.brand} ${item.name} (${item.collection}) — ${item.quantity} шт.${configuration}`;
    }).join("\n");
