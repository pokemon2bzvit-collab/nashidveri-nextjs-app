...
import { LoaderCircle, Send, ShoppingBag, Trash2, X } from "lucide-react";
...
export function CartDrawer() {
  ...
  async function submit(values: Values) {
    const models = items.map((item, index) => `${index + 1}. ${item.brand} ${item.name} (${item.collection})`).join("\n");
    const message = [values.message?.trim(), "Товари для прорахунку:", models].filter(Boolean).join("\n\n").slice(0, 1200);
    setServerError("");
    const response = await fetch("/api/leads", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...values, message, requestType: "price_request", productName: `Кошик: ${items.length} ${items.length === 1 ? "модель" : "моделей"}`, sourcePath: window.location.pathname }) });
